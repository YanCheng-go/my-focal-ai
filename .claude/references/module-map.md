...
- `src/ainews/ingest/__init__.py` — shared utilities: `SCRAPER_HEADERS`, `utc_today()`, `rank_to_score()`, `MAX_TRENDING_ITEMS`.
- `src/ainews/ingest/` — fetches from all sources. `feeds.py` for RSS/Atom (incl. XHS via RSSHub), `twitter.py` for Twitter via Chrome cookies + GraphQL, `events.py` for scraping tech company event pages (Anthropic, Google), `github_trending.py` for trendshift.io scraping, `aitmpl_trending.py` for aitmpl.com AI tools trending (snapshot), `skillssh_trending.py` for skills.sh agent skills trending (snapshot + audit badges), `runner.py` orchestrates and skips existing items.
